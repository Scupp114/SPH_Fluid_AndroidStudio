...
#ifdef Fluid3d
struct hash_func : unary_function<ivec3, size_t> {
    size_t operator()(ivec3 const& pos) const {
        return(size_t) ((pos.x * 73856093) ^ (pos.y * 19349663) ^ (pos.z * 83492791));
    }
};
typedef unordered_map<ivec3,vector<Particle*>,hash_func> hmap_t;
struct Hasher{
    hmap_t map;
    vector<ivec3> offs;
    GLfloat sizeInv;
    ...
    Hasher(cuint numBuckets, GLfloat cellSize) : map(numBuckets), sizeInv(1.f/cellSize){
        for(int i=-1; i<=1; i++)
            for(int j=-1; j<=1; j++)
                for(int k=-1;k<1;k++)
                    offs.push_back(ivec3(i, j, k));
    }
    void insert(const vec3& pos, Particle* p ){map[toBucket(pos)].push_back(p);}
    void clear(){map.clear();}
    inline ivec3 toBucket(const vec3& pos)const {return ivec3(floor(pos*sizeInv));}

    void make_nlist(vec3& pos, vector<Particle*>& out) const {
        const ivec3 bkt = toBucket(pos);
...
struct Fluid{
    ...
    vec3 gravity_dir;
    Fluid():window({0,0,eglGetCurrentContext()}),gravity_dir(0.0,-1.0, 0.0){}
    void resize(int w, int h);
    void display();
    bool init(cuint);
    void step();
    void update_gravity(float theta){
        gravity_dir = vec3(sin(theta),-cos(theta), 0.0); // don't no why updating gravity
    }
